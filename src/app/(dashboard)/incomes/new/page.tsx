...
const NewIncomePage = async () => {
  ...
  const data: IncomeFormData = {
    ...
    customers,
    employees: availableUsers.map((candidate) => ({ id: candidate.id, firstName: candidate.firstName, lastName: candidate.lastName, role: candidate.role.name, isActive: candidate.isActive, serviceCommissionRate: 0, productCommissionRate: 0 })),
  };
