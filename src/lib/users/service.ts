...
export const createUser = async (actor: SafeUser, input: CreateUserInput, dependencies = defaultDependencies) => {
  ...
  const ownerCommissionRate = input.roleId === 1 ? 0 : null;
  return dependencies.users.create({
    firstName: input.firstName,
    lastName: input.lastName,
    passwordHash,
    roleId: input.roleId,
    serviceCommissionRate: ownerCommissionRate ?? input.serviceCommissionRate,
    productCommissionRate: ownerCommissionRate ?? input.productCommissionRate,
    createdBy: actor.id,
  });
};
...
export const updateUser = async (
  ...
  const normalizedChanges: UpdateUserInput = (changes.roleId ?? target.role.id) === 1
    ? { ...changes, serviceCommissionRate: 0, productCommissionRate: 0 }
    : changes;
  const updated = await dependencies.users.update(id, normalizedChanges);
