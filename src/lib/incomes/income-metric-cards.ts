...
export const buildIncomeMetricCards = (metrics: IncomeListMetrics, role: UserRole): IncomeMetricCard[] => role === "employee" ? [
  ...
  { label: "Mi comisión", value: metrics.commissionTotal === undefined ? unavailable : formatArs(metrics.commissionTotal), detail: metrics.commissionTotal === undefined ? "Pendiente de backend" : "Comisión devengada", tone: "primary" },
  ...
  { label: "Comisiones", value: metrics.commissionTotal === undefined ? unavailable : formatArs(metrics.commissionTotal), detail: metrics.commissionTotal === undefined ? "Pendiente de backend" : "Comisiones devengadas", tone: "primary" },
  { label: "Neto barbería", value: metrics.barbershopNet === undefined ? unavailable : formatArs(metrics.barbershopNet), detail: metrics.barbershopNet === undefined ? "Pendiente de backend" : "Facturado menos comisiones", tone: "light" },
