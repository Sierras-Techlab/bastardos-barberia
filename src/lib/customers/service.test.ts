...
import { createCustomer, deleteCustomer, listCustomers, updateCustomer } from "@/lib/customers/service";
...
const deps = (): CustomerServiceDependencies => ({
  customers: {
    list: vi.fn().mockResolvedValue([customer]), findById: vi.fn().mockResolvedValue(customer),
