...
export const FixedCustomersCard = ({ occurrences, onStatusChange }: Props) => {
  const [currentOccurrences, setCurrentOccurrences] = useState(() => sortFixedOccurrences(occurrences));
  ...
  return <section aria-labelledby="fixed-customers-title" className="h-fit rounded-3xl border border-black/5 bg-white p-4 shadow-[0_20px_55px_-42px_rgba(0,0,0,0.4)] sm:p-5">
    ...
    })}</ul> : <div className="mt-4 flex min-h-40 flex-col items-center justify-center rounded-2xl border border-black/5 bg-[#f7f6f3] px-5 text-center"><UserRoundCheck className="size-7 text-primary" /><p className="mt-3 font-semibold">No hay turnos fijos para el resto de la semana</p><p className="mt-1 text-sm text-muted-foreground">Podés asignar un horario semanal desde el directorio.</p><Link href="/customers" className={cn(buttonVariants({ variant: "outline", size: "sm" }), "mt-3 rounded-xl")}>Gestionar clientes</Link></div>}
