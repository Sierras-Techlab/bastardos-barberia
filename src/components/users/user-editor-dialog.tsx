...
export const UserEditorDialog = ({
  ...
  const [serviceCommissionRate, setServiceCommissionRate] = useState(
    String(commissionUser?.serviceCommissionRate ?? 0),
  );
  const [productCommissionRate, setProductCommissionRate] = useState(
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (parsedServiceCommissionRate !== commissionUser?.serviceCommissionRate) {
      changes.serviceCommissionRate = parsedServiceCommissionRate;
    }
    if (parsedProductCommissionRate !== commissionUser?.productCommissionRate) {
      changes.productCommissionRate = parsedProductCommissionRate;
    }
              ...
              <div className="grid gap-4 rounded-2xl bg-[#f7f6f3] p-4 sm:col-span-2 sm:grid-cols-2">
                <label className="space-y-1.5 text-sm font-medium">
                  ...
                  <Input
                    ...
                    onChange={(event) => setServiceCommissionRate(event.target.value)}
                    className={fieldClassName}
                  />
                </label>
                <label className="space-y-1.5 text-sm font-medium">
                  ...
                  <Input
                    ...
                    onChange={(event) => setProductCommissionRate(event.target.value)}
                    className={fieldClassName}
                  />
                </label>
