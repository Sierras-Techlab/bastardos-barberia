...
import { CircleCheck, Loader2, ScrollText } from "lucide-react";
...
export type CashCloseDialogProps = {
  expectedCash: number;
  ...
  return <Dialog open onOpenChange={(open) => !open && !submitting && onClose()}>
    <DialogContent className="max-h-[calc(100svh-2rem)] overflow-y-auto rounded-[1.6rem] p-5 sm:max-w-md">
      <form noValidate onSubmit={submit} className="space-y-5">
        <DialogHeader>
          <span className="mb-1 flex size-10 items-center justify-center rounded-2xl bg-red-50 text-primary"><ScrollText className="size-5" /></span>
          <DialogTitle>{mode === "manual" ? "Cerrar caja con conteo" : "Confirmar conteo automático"}</DialogTitle>
          <DialogDescription>El cierre congela los totales financieros; solo podés ajustar el conteo físico.</DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="-mx-5 -mb-5 flex flex-row items-center justify-end gap-2 p-5">
          ...
          <Button type="submit" disabled={submitting} className="rounded-xl">
            ...
            {mode === "manual" ? (difference === 0 ? "Cerrar y confirmar" : "Cerrar y registrar diferencia") : "Confirmar conteo"}
          </Button>
        </DialogFooter>
      </form>
    </DialogContent>
