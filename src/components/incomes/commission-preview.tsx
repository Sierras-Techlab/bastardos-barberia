...
export const CommissionPreview = ({
  ...
  const commissionApplies = employee?.role !== "owner";
  const preview = calculateCommissionPreview({
    serviceBase,
    productBase: total - serviceBase,
    serviceRate: commissionApplies ? employee?.serviceCommissionRate ?? 0 : 0,
    productRate: commissionApplies ? employee?.productCommissionRate ?? 0 : 0,
    grantFullServiceCommission: commissionApplies && values.grantFullServiceCommission,
  });
  const manager =
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-xs text-muted-foreground">
        Servicio {preview.serviceRate}% · Productos {preview.productRate}%
      </p>
