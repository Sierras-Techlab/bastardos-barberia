...
export const IncomesView = ({ data, initialQuery, currentUser, employees, canViewAll, canVoid, incomeClient = defaultIncomeClient }: Props) => {
  const initialFilters = { ...asFilters(initialQuery), ...(!canViewAll && { employeeId: "" }) }; const [filters, setFilters] = useState(initialFilters); const [result, setResult] = useState(data); const [selected, setSelected] = useState<IncomeListItem | null>(null); const [voiding, setVoiding] = useState<IncomeListItem | null>(null); const [loading, setLoading] = useState(false); const [error, setError] = useState<string | null>(null); const requestSequence = useRef(0);
  ...
  return <div className="space-y-5"><IncomeMetrics metrics={result.metrics} role={currentUser.role} /><IncomeFilters role={currentUser.role} canFilterEmployees={canViewAll} employees={employees} value={filters} onChange={changeFilters} onClear={() => changeFilters(initialFilters)} canClear={canClear} />
