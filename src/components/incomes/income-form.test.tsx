...
const result = (input: CreateIncomeInput): Income => ({ id: "20000000-0000-4000-8000-000000000001", createdAt: "2026-08-11T12:00:00.000Z", businessDate: "2026-08-11", employee: data.currentUser, customer: null, service: data.services[0], products: [], paymentMethod: input.paymentMethod, total: 13000, status: "active" });
...
describe("IncomeForm", () => {
  it("always displays the authenticated user without a selector", () => {
  ...
  it("sends a generated request id and no actor, total or date", async () => {
    ...
    expect(input.requestId).toMatch(/^[0-9a-f-]{36}$/); expect(input).not.toHaveProperty("employeeId"); expect(input).not.toHaveProperty("total"); expect(input).not.toHaveProperty("createdAt");
