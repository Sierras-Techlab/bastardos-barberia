...
type CustomerEditorDialogProps = {
  ...
  customers: Customer[];
  onClose: () => void;
  onSave: (input: CustomerEditorInput) => void;
};
      ...
      <DialogContent className="max-h-[calc(100svh-2rem)] overflow-y-auto rounded-[1.6rem] p-5 sm:max-w-lg">
        <form noValidate onSubmit={submit}>
          <DialogHeader>
            ...
            <DialogDescription>Los datos de contacto son obligatorios y deben ser únicos.</DialogDescription>
          </DialogHeader>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <label className="space-y-1.5 text-sm font-medium sm:col-span-2">Email<Input type="email" value={email} onChange={(event) => setEmail(event.target.value)} className={fieldClassName} /></label>
          ...
          <DialogFooter className="-mx-5 -mb-5 mt-5 p-5">
            ...
            <Button type="submit" className="rounded-xl">{mode === "create" ? "Crear cliente" : "Guardar cambios"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
