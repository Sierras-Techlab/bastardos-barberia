...
import { serviceClient as defaultServiceClient, type ServiceClient } from "@/lib/services/client";
...
export const ServicesView = ({ data, canManage, serviceClient = defaultServiceClient }: {
  ...
  const replaceService = (updated: ServiceCatalogItem) =>
    setCatalog((current) => current.map((service) => service.id === updated.id ? updated : service));
  const saveService = async (input: ServiceEditorInput) => {
    if (editor?.mode === "edit" && editor.service) {
      const updated = await serviceClient.update(editor.service.id, input);
      replaceService(updated);
      toast.success("Servicio actualizado correctamente.");
    } else {
      const created = await serviceClient.create(input);
  ...
  return <div className="space-y-5">
    ...
    {statusService && <ServiceStatusDialog service={statusService} onClose={() => setStatusService(null)} onConfirm={async () => {
      const updated = await serviceClient.update(statusService.id, { isActive: !statusService.isActive });
      replaceService(updated);
      toast.success(statusService.isActive ? "Servicio desactivado correctamente." : "Servicio activado correctamente.");
      setStatusService(null);
    }} />}
    {deleteService && <ServiceDeleteDialog service={deleteService} onClose={() => setDeleteService(null)} onConfirm={async () => {
      await serviceClient.remove(deleteService.id);
      setCatalog((current) => current.filter((service) => service.id !== deleteService.id));
