...
export type IncomeFormData = { currentUser: CurrentUser; customers: Customer[]; services: Service[]; products: Product[]; employees?: Employee[] };
...
export type IncomeListItem = {
  ...
  service: Service | null; products: IncomeListProduct[]; paymentMethod: PaymentMethod; total: number; status: IncomeStatus;
};
...
export type IncomeListMetrics = { total: number; count: number; average: number; cashTotal: number; transferTotal: number };
