...
| Reports | In development | Colleague-owned business reports use migration `039` and must preserve the authoritative income, expense and Caja contracts. |

## Current product objective

The active branch is `feat/changes-fullstack`, synchronized with current `origin/dev` before production hardening. The configured test database already has the behavior now owned by `040_production_hardening.sql`; the source sequence reserves `039` for the colleague-owned Reports migration. Repeatable direct-PostgreSQL, authenticated-HTTP and multi-connection acceptance verify RLS/grants, canonical RPCs, authentication/session lifecycle, administrative catalogs, inventory, customers/schedules, presentismo, role-aware incomes, overrides, commissions, fixed monthly payments, live and closed Caja projections, expenses and critical stock/session/idempotency races. The complete `001`-`040` clean-install gate must be rerun after the Reports file is integrated. Production remains gated on target backup, read-only preflight and deployment approval.
