...
| Dashboard UI | Implemented locally | Real role-scoped income summary, quick actions and persisted fixed-customer occurrences with attendance limited to the remaining current Monday-through-Saturday week. The income summary card reads the sanitized employee projection for employees and the full manager economics otherwise. |
| Comisiones y pagos combinados | Implemented locally | Role-aware responsible employee, exact split payments, independently rounded item snapshots, owner-safe previews, audited 100% service/product-line exceptions, manager-controlled charged-price overrides with reason, employee basis-point payment entry, sanitized employee projections; migrations 010 through 015 remain pending manual installation and migration 020 awaits real PostgreSQL acceptance. |
| Historial de ingresos por rol | Implemented locally | Responsible-employee scoping/filtering across all historical users, role-aware metrics, payments, commissions, registrant audit and full detail. The client parses the API envelope with the manager or sanitized employee schema by viewer role. |
| Income entry UI | Implemented | Real authenticated sale submission with active catalogs, server-authoritative totals, role-safe idempotency and inline customer creation. |
| Income history UI | Implemented | Role-scoped server filtering, monthly pagination, filtered metrics, read-only detail and manager-only voiding. The employee viewer never receives catalog/charged prices, payments, barbershop net or registered-by fields. |
...
| Sales and cash | Implemented locally | Persistent sales feed a manager-only Caja workspace. Migration `022` persists a live register, adds manual open/close/confirm with automatic first-income opening and automatic pending-confirmation closing, and snapshots normal sales plus fixed subscriptions before closure. Manual closes are confirmed immediately even with a difference; reconciliation of an automatic close never recomputes financial snapshots. Physical expected cash uses only opening balance, active `system_code='cash'` payments and cash adjustments. Migration `018` historical snapshots remain immutable. Expenses remain separate. |
...
| Fixed-customer monthly payments | Implemented locally | Manager-controlled monthly price and professional assignment, role-aware collection (manager amount / employee basis points), atomic subscription income with append-only attempts, employee-safe financial projections, void-reopens-month behaviour and dashboard/cash integration; the canonical void_income RPC keeps its existing audit semantics and an AFTER UPDATE OF status trigger flips the linked attempt; migration 021 awaits real PostgreSQL acceptance. |
| Reports | Planned | Derive revenue and operating reports from real transactional data. |

## Current product objective

The resumable status and execution order for the complete operational-control roadmap is documented in `docs/superpowers/plans/2026-08-22-operational-control-roadmap-status.md`. The corrective repair plan `docs/superpowers/plans/2026-08-23-operational-control-corrective-repair.md` has all six tasks closed locally on `feat/changes-fullstack`: role-safe income entry, role-aware history/dashboard projections, rebuilt migration 021 monthly-payment contract (status check + partial unique active index + AFTER UPDATE OF status trigger + employee projection without `monthlyPrice`), reconciled 021 client and UI contracts (strict Zod envelopes, integer ARS, no `/100`/`*100` conversions, `FixedCustomerPaymentApiError`), rebuilt migration 022 Caja (`system_code` ordering, relaxed counts check, named PL/pgSQL variables, Efectivo-only expected cash, universal open trigger), behavioural regression cases for migration 023 and full verification. Real PostgreSQL acceptance (Task 6) cannot be executed from this chat; the cumulative 019 → 023 chain must remain un-deployed until the operator records the disposable-project acceptance evidence.

The approved next roadmap is split into five deployable blocks: `019` employee work sessions and presentism; `020` manager price overrides, configurable owner commission and employee-safe financial projections; `021` professional-owned fixed customers with atomic monthly payments; `022` manual cash opening/counting with automatic pending-confirmation fallback; and `023` customer last visit derived from active normal sales. Blocks `019` through `023` are implemented locally across migration, domain, authenticated API and responsive UI; their migrations remain pending manual installation in the configured Supabase project and their acceptance scenarios must be run against a disposable database before production. Block `021` adds `responsible_user_id` + `monthly_price` to fixed schedules, atomic monthly collection via `pay_fixed_customer_month`, append-only `fixed_customer_monthly_payment_attempts` with a `status` check + partial unique active index, employee-safe financial projections, a manager professional selector inside the customer editor and a `Cobrar mensualidad` action scoped to the active professional. Customer visit totals and the `visits` counter continue to count only active normal sales; subscription incomes affect Caja and dashboards without inflating visit history.

The installed baseline through `018` keeps Caja read-only and automatic: it derives live totals from incomes, exposes dynamic payment totals and sale-level audit, skips empty dates and never asks a manager to open or close the day. The `022` block evolves that same canonical model by adding manual open/close/confirm with automatic first-income opening and automatic pending-confirmation closing; reconciliation never recomputes closed snapshots.
