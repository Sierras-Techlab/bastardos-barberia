...
| User administration API | Installed in test | Create, list, inspect, update, activate/deactivate, reset passwords and logically delete, including configurable owner commissions and the canonical profile RPC. |
| Dashboard UI | Installed in test | Real role-scoped income summary, quick actions and persisted fixed-customer occurrences for the remaining current Monday-through-Saturday week. Employees receive and may resolve only occurrences currently assigned to them; managers retain the complete agenda. |
| Comisiones y pagos combinados | Implemented | Role-aware responsible employee, exact split payments, independently rounded item snapshots, owner-safe previews, audited 100% service/product-line exceptions, service charged-price adjustments with reason for employees and managers, manager-only product price adjustments, employee percentage payment entry and employee-sanitized history projections. |
...
| Sales and cash | Implemented | Persistent sales feed a manager-only Caja workspace with automatic opening/closing, an opening balance editable before or during sales, and physical-count confirmation after automatic close. Historical snapshots remain immutable, `Efectivo` is protected by `system_code='cash'`, and expenses remain separate. |
...
| Customers | Installed in test | Persistent directory, optional weekly schedule with responsible professional and monthly price, sale-only financial visit detail, audited employee-scoped attendance and query-derived last visit. Migration `040` converges clean and upgraded authorization/visit semantics. |
| Fixed-customer monthly payments | Installed in test | Manager-controlled monthly price and professional assignment, role-aware collection, atomic subscription income with append-only attempts, employee-safe projections, void-reopens-month behaviour and dashboard/Caja integration without affecting customer visits. |
| Expenses | Installed in test | Manager-only monthly operating view with authoritative income/net/result summary, filtered expense ledger, audited create/edit/void flows, optional administrative payment method and recoverable category lifecycle. Expenses do not alter Caja in MVP. |
| Reports | Installed in test | Manager-only visual monthly reporting with financial comparison, projection, daily evolution, compositions, service/product highlights and team production. Team rows combine responsible-user sales, commissions, net, ticket, attendance productivity and outside-session sales. Migration `039` preserves the authoritative income, expense and Caja contracts. |

## Current product objective

The active branch is `dev`, with Reports `039`, production hardening `040`, the responsive hardening and the application work for `041` integrated locally. Migration `041` still requires authorized installation and PostgreSQL acceptance. Repeatable direct-PostgreSQL, authenticated-HTTP and multi-connection acceptance verify RLS/grants, canonical RPCs, authentication/session lifecycle, administrative catalogs, inventory, customers/schedules, presentismo, role-aware incomes, overrides, commissions, fixed monthly payments, live and closed Caja projections, expenses, reports and critical stock/session/idempotency races. The income ledger now also prevents stale metrics from being presented under a failed filter and provides bounded loading with retry. The complete `001`-`041` clean-install gate must pass before production. Production remains gated on target backup, read-only preflight and deployment approval.

The operational-control roadmap comprises `019` employee work sessions, `020` price overrides/configurable owner commission/employee-safe projections, `021` professional-owned fixed customers with atomic monthly payments, historical `022` manual Caja lifecycle and `023` derived customer last visit. These blocks are installed in the test database; `041` supersedes the manual controls once deployed. Customer visit totals continue to count only active normal sales; subscription incomes affect Caja and dashboards without inflating visit history.

Caja opens and closes automatically. Managers may load or correct today's opening balance before or during sales, but cannot open or close the register manually. An automatic close remains pending until a manager confirms the physical count; reconciliation never recomputes closed financial snapshots. Historical manual lifecycle records remain readable.
