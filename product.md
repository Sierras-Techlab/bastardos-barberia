...
| Reports | Design approved | Manager-only visual business-performance report: equivalent monthly comparison, labeled projection, financial result, daily evolution, income/payment/expense composition and service/product highlights. Implementation is specified but not started. |
...
The first Reportes increment is approved in `docs/superpowers/specs/2026-08-25-business-reports-design.md` and planned in `docs/superpowers/plans/2026-08-25-business-reports.md`. It starts from merged `dev`, remains manager-only and explains business performance without duplicating Caja reconciliation or exposing employee/customer identities. Implementation and migration `039` have not started.
