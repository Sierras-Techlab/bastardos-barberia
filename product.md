...
| Presentismo | Persistence implemented locally | Migration `019` records employee clock-in/out, audited manager corrections and server-derived income linkage with role-specific metrics. Authenticated API and UI remain pending. |
...
The approved next roadmap is split into five deployable blocks: `019` employee work sessions and presentism; `020` manager price overrides, configurable owner commission and employee-safe financial projections; `021` professional-owned fixed customers with atomic monthly payments; `022` manual cash opening/counting with automatic pending-confirmation fallback; and `023` customer last visit derived from active normal sales. The domain and persistence layers of `019` are implemented locally; its authenticated API and UI remain pending, and migration `019` has not been applied remotely. The current `012` owner-zero and `018` automatic read-only Caja behavior remains authoritative until its matching block is installed.
